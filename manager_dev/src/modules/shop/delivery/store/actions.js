...
const actions = {
  ...
  async create({
    ...
    if (response !== undefined && response.status === 200) {
      ...
      const data = requestDataHandler('GET', `/deliveries/find/${response.data.id}`, undefined, {
        filter: {
  ...
  async update({
    ...
    this.dispatch('preloader/fetch', true);
    const data = requestDataHandler('PUT', '/deliveries/update', payload.body);
  ...
  async remove({
    ...
    this.dispatch('preloader/fetch', true);
    const data = requestDataHandler('DELETE', '/deliveries/remove', payload.body);
