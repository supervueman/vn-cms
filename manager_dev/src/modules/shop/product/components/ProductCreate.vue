<template lang="pug">
  v-layout(v-if="r.is_product_create")
    v-flex
      v-layout.wrap.pt-4
              ...
              v-layout.wrap
                v-flex.md12
                  v-menu(
                    ...
                    :nudge-right="40"
                    :return-value.sync="product.createdAt"
                    lazy
          ...
          v-card.mb-3
            v-card-text
              v-layout.wrap
                v-flex.md12
                  Editor(
                    v-if="product.content !== undefined"
      ...
      v-card
        v-card-actions
          v-btn.ml-2(
            color="primary"
            v-if="r.is_product_create"
            @click="create"
          ) {{d.create}}
          v-btn.ml-2(
            @click="$emit('close')"
            v-if="product.parentId !== '' && product.parentId !== undefined"
          ) {{d.cancel}}
</template>
...
export default {
  ...
  data() {
    return {
      ...
      panelName: "panel-product-create",
      menu: false
    };
  },
  ...
  methods: {
    create() {
      ...
      this.$v.$touch();
      if (!this.$v.$error) {
        this.$store.dispatch("shop/product/createProduct", this.product);
      }
    }
  }
};
